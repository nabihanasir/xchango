        ...
        <div className="mt-12 grid gap-6 xl:grid-cols-3">
          {filteredPartners.map((partner) => {
            ...
            return (
              <article
                key={partner.id}
                className="group relative flex h-full flex-col overflow-hidden rounded-[2rem] border border-slate-200/70 bg-white p-6 shadow-soft transition duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-200/60"
              >
                <img
                  src={`https://flagcdn.com/w320/${partner.flag.toLowerCase()}.png`}
                  alt=""
                  aria-hidden="true"
                  loading="lazy"
                  className="pointer-events-none absolute inset-0 h-full w-full object-cover opacity-[0.07] transition duration-300 group-hover:opacity-[0.13]"
                />
                <div className="relative flex flex-1 flex-col">
                  ...
                  <div className="mt-6">
                    <p className="text-xs font-black uppercase tracking-[0.28em] text-black">Programs</p>
