...
export default function AdminUniversities() {
  ...
  const columns = [
    ...
    { header: 'Website', accessor: 'website', render: (val: string) => val ? <a href={val} target="_blank" rel="noreferrer" className="text-maroon hover:underline">{val}</a> : 'N/A' },
  ...
  return (
    <div className="space-y-6 animate-fade-in relative z-0">
       <div className="flex items-center justify-between border-b border-light-color/50 pb-6 glass-card p-6 md:p-8 rounded-[2rem] bg-white">
        <div>
          <h2 className="text-3xl font-black text-maroon mb-2">Universities Catalog</h2>
        ...
        <div className="flex gap-3">
           <button 
             ...
             className="flex items-center gap-2 px-5 py-2.5 bg-maroon-gradient text-white font-bold rounded-xl hover:brightness-110 transition-all shadow-lg shadow-maroon/20"
           >
      ...
      <div className="glass-card rounded-[2rem] bg-white p-6 md:p-8 shadow-sm">
        {loading ? (
          ...
          <div className="text-center py-12 px-6 border-2 border-dashed border-light-color rounded-2xl bg-slate-50">
            ...
            <h3 className="text-xl font-bold text-maroon">No Universities Found</h3>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-maroon/80 backdrop-blur-md" onClick={() => !creating && setIsModalOpen(false)} />
          <div className="bg-white w-full max-w-lg rounded-[32px] shadow-2xl relative z-10 overflow-hidden animate-fade-in-up border border-white/20">
            <header className="px-8 py-6 border-b border-light-color/60 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <h2 className="text-2xl font-black text-maroon">New University</h2>
              </div>
              <button 
                onClick={() => setIsModalOpen(false)}
                disabled={creating}
                className="p-2 hover:bg-slate-100 rounded-xl transition-all text-maroon/40 hover:text-maroon"
              >
                <X className="h-5 w-5" />
              </button>
            </header>
            ...
            <form onSubmit={handleCreate} className="p-8 space-y-5">
              <div>
                <label className="block text-xs font-black text-maroon/60 uppercase tracking-widest mb-2">University Name</label>
                <input 
                  type="text" 
                  required
              ...
              <div>
                <label className="block text-xs font-black text-maroon/60 uppercase tracking-widest mb-2">Country</label>
                <select 
                  required
              ...
              <div>
                <label className="block text-xs font-black text-maroon/60 uppercase tracking-widest mb-2">Website URL (Optional)</label>
                <input 
                  type="url" 
              ...
              <div>
                <label className="block text-xs font-black text-maroon/60 uppercase tracking-widest mb-2">Seat Limit</label>
                <input 
                  type="number" 
              ...
              <div className="pt-4 flex justify-end gap-3">
                ...
                <button 
                  type="submit" 
                  disabled={creating}
                  className="px-6 py-2.5 bg-accent-yellow text-maroon font-bold rounded-xl hover:bg-yellow-default transition-all shadow-lg shadow-accent-yellow/20 disabled:opacity-50"
                >
