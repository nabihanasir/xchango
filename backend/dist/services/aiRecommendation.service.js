...
exports.getCourseRecommendations = void 0;
...
const tokenize = (value) => Array.from(new Set(value
    .toLowerCase()
    .split(/[^a-z0-9]+/)
    .map((part) => part.trim())
    .filter((part) => part.length > 2 && !STOP_WORDS.has(part))));
const collectCourseTokens = (course) => tokenize([course.name, course.code, course.description || '', course.outlineText || ''].join(' '));
const getOverlap = (left, right) => left.filter((item) => right.includes(item));
const toPercent = (value) => Math.max(0, Math.min(100, Math.round(value)));
const getGpaBoost = (gpa, courseTokens) => {
    const isAdvancedCourse = courseTokens.some((token) => ['advanced', 'analysis', 'research', 'algorithm', 'machine', 'intelligence'].includes(token));
    if (gpa >= 3.7) {
...
const buildReason = ({ backgroundOverlap, interestOverlap, transcriptOverlap, gpa, matchScore, }) => {
    const reasons = [];
...
const getCourseRecommendations = (studentProfile, availableCourses) => {
